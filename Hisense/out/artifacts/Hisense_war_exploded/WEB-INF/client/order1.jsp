                                        ...
                                        <div class="selectric-wrapper selectric-select selectric-trade-status-select">
                                            <div class="selectric-select">
                                                <select class="select trade-status-select" name="orderStatus" tabindex="-1" style="height: 30px;padding:2px 5px;font-size:15px;">
                                                    <option value="0" <c:if test="${coder.orderStatus eq ''}">selected="selected"</c:if> >全部</option>
                                                    <option value="1" <c:if test="${coder.orderStatus eq '1'}">selected="selected"</c:if>>已收货</option>
                                                    <option value="3" <c:if test="${coder.orderStatus eq '3'}">selected="selected"</c:if>>已取消</option>
                                                    <option value="11" <c:if test="${coder.orderStatus eq '11'}">selected="selected"</c:if>>申请售后</option>
                                                    <option value="12" <c:if test="${coder.orderStatus eq '12'}">selected="selected"</c:if>>已退款</option>
                                                </select>
                                            </div>
                                    ...
                                    <span class="span-ib fr">
                                        ...
                                        <div class="selectric-wrapper">
                                            ...
                                            <select name="orderTime" id="date-select-dropdown" tabindex="-1" style="height: 30px;padding:2px 5px;font-size:15px;">
                                                <option value="" <c:if test="${coder.orderTime eq ''}">selected="selected"</c:if>>请选择</option>
                                                <option value="1" <c:if test="${coder.orderTime eq '1'}">selected="selected"</c:if>>最近一个月</option>
                                                <option value="3" <c:if test="${coder.orderTime eq '3'}">selected="selected"</c:if>>最近三个月</option>
                                                <option value="6" <c:if test="${coder.orderTime eq '6'}">selected="selected"</c:if>>最近六个月</option>
                                                <option value="12" <c:if test="${coder.orderTime eq '12'}">selected="selected"</c:if>>最近十二个月</option>
                                            </select></div>
                                            <input class="selectric-input" tabindex="0">
                                        </div>
                                ...
                                <tr class="order-info-tr">
                                    ...
                                    <td class="order-info-status" width="500">
                                        <c:if test="${olist.ostate == 1}">
                                            <span>已收货</span>
                                            <a href="<%=request.getContextPath()%>/client/EvaluateServlet?method=gotoEvaluate&oid=${olist.oid}" class="" target="_top" data-id="">去评价</a>
                                        </c:if>
                                        ...
                                        <c:if test="${olist.ostate == 5}">
                                            <span>已发货</span>
                                            <a href="" class="updateOrderBystatus" data-id="${olist.oid}">确认收货</a>
                                        </c:if>
                                        ...
                                        <span>待支付</span>
                                        <!-- 买家订单操作-->
                                        <a class="js-pay pay-order" target="_top" href="<%=request.getContextPath()%>/client/OrderServlet?method=getOrderByoid&oid=${olist.oid}&aid=${olist.aid}" data-id="2008080004691313">立即付款</a>
                                        <a href="" class="updateOrder" data-id="${olist.oid}">取消订单</a>
                                        ...
                                        <div class="sku-order " data-id="2007100000025613">
                                            <div class="sku-order-item order-item-info">
                                                <div class="item-thumbnail ">
                                                    <a href="<%=request.getContextPath()%>/client/CommodityServlet?method=getCommodityDetail&coid=${orderItem.commodity.coid}" target="blank">
                                                ...
                                                <div class="item-desc ">
                                                    <span class="item-desc-name">
                                                         <a class="cutwords-two-line" href="<%=request.getContextPath()%>/client/CommodityServlet?method=getCommodityDetail&coid=${orderItem.commodity.coid}" target="blank">
